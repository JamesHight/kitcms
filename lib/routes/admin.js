...
app.get('/admin', basicAuth, function(req, res, next) {
	db.keys(req.domain.namespace, function(err, keys) {
		keys = keys.sort();
...
app.all('/admin/get', basicAuth, function(req, res, next) {
	...
	db.get(req.domain.namespace + '&' + key, function(err, doc) {
		res.set('Content-Type', 'application/json');
...
app.all('/admin/set', basicAuth, function(req, res, next) {
	...
	db.set(req.domain.namespace + '&' + key, value, function(err, keys) {
		res.set('Content-Type', 'application/json');
...
app.all('/admin/unset', basicAuth, function(req, res, next) {
	...
	db.unset(req.domain.namespace + '&' + key, function(err, keys) {
		res.set('Content-Type', 'application/json');
		res.send(JSON.stringify('OK'));
	});
});
...
app.all('/admin/import', basicAuth, function(req, res, next) {
	...
	fs.readFile(req.files.file.path, 'utf8', function(err, data) {
		...
		function processEntry(entry, cb) {
			...
			db.set(req.domain.namespace + '&' + entry.key, entry.value, cb);
		}
